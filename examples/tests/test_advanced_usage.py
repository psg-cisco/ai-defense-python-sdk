...
def test_advanced_usage_workflow(capsys):
    ...
    dummy_result = MagicMock(is_safe=True, rules=None, classifications=None, explanation=None, event_id="evt-123", severity=None)
    dummy_http_result = MagicMock(is_safe=True)

    with patch.object(ChatInspectionClient, 'inspect_prompt', return_value=dummy_result), \
         patch.object(ChatInspectionClient, 'inspect_conversation', side_effect=ValidationError("Validation error: Empty conversation")):
        # Custom rules example
        print("\n=== Custom Rules and Entity Types Example ===")
        config = InspectionConfig(
            enabled_rules=[
                Rule(rule_name=RuleName.PII, entity_types=["EMAIL", "PHONE_NUMBER", "CREDIT_CARD"]),
                Rule(rule_name=RuleName.PROMPT_INJECTION),
            ]
        )
        client = ChatInspectionClient(api_key=dummy_api_key)
        result = client.inspect_prompt("My email is john.doe@example.com and my phone is 555-123-4567", config=config)
        ...
        result = client.inspect_prompt("How to hack into a computer system? Also, here's my SSN: 123-45-6789")
        if result.is_safe:
        ...
        result2 = client.inspect_prompt("This is a prompt with custom timeout", timeout=15)
