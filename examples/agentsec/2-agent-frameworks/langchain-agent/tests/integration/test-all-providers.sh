...
show_help() {
    ...
    echo "  $0 --gateway            # Test all providers in Gateway mode only"
}
...
test_provider_with_mode() {
    ...
    if grep -q "Agent:" "$log_file" || grep -q "final response\|Response:" "$log_file"; then
...
# Setup
log_header "LangChain Agent Integration Tests"
echo ""
