...
"""
FastAPI application for GKE deployment with LangChain Agent.

This file provides a REST API for the SRE agent when deployed to Google Kubernetes Engine.
GKE provides full control over infrastructure with Kubernetes orchestration.
...
app = FastAPI(
    title="Vertex AI SRE Agent - GKE",
    description="LangChain-based SRE Agent with Cisco AI Defense protection, deployed on GKE",
    version="1.0.0",
)
...
def health():
    ...
    return {"status": "healthy", "service": "sre-agent-gke"}
